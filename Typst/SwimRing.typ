...
9. 经过两次分割，圆环面被分割成了Phi--num*Polar-num个小型的四顶点曲面块。
10. 这些小型的四顶点曲面块由两组平行的对边组成，两组对边都是相互平行的圆弧段，理论上可以被精确绘制。
11. 但是，由于我不知道怎样把这四个独立的圆弧段拼接成一个完整的、封闭的曲面，这会妨碍我之后对这些曲面进行颜色填充。
12. 所以，精确绘制圆弧段的方式将只有描边线，无法进行颜色填充。
13. 另一种方案是，使用由这四个顶点构成的矩形块近似替代这些微小曲面。
14. 可以想象通过这种方式绘制的圆环面是由许多个矩形块拼接而成的，最终结果并不是光滑的曲面。
15. 通过对矩形块的两个对角线向量做叉乘，求解每一个矩形块的法线方向。
16. 通过截面圆心到矩形块中心位置的向量与矩形块法线方向之间的夹角来求解矩形块的外法线方向。
17. 使用平行光线模拟太阳光线，使用Lambert's cosine law来计算平行光线的反射光强度，叠加环境光以避免全黑阴影。
18. 将总的反射光强度映射为深浅不一的颜色值。
19. 设置描边颜色
20. 需要注意分割方式，因为polar与theta并不是两个独立的分量,theta分量的位置完全由polar分量决定。
21. 分割时必须使用polar分量，因为只有它能线性对应内管的周长。
22. 选定polar分量以后，再选独立分量，发现只有phi分量可选。
*/
