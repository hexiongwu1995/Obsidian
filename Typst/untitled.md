...
\draw[->](0,0,0)--(4,0,0)node[font=\large,right]{x};
\draw[->](0,0,0)--(0,4,0)node[font=\large,above]{y};
\draw[->](0,0,0)--(0,0,4)node[font=\large,left]{z};
\end{tikzpicture}
\end{document}
```
...
for j in linspace(0, 2* calc.pi, n:500) {
rotate(y:j*1rad, origin:(0,0,0))
on-xy(z:0, circle((4,0), radius:0.5, stroke:(paint:green.transparentize(80%), thickness:10pt)))
